...
## 2026-07-22 (Physics formula card)

- По явному запросу создана отдельная учебная заметка [[Physics/Формулы — математический аппарат физики|Формулы — математический аппарат физики]].
- В карточке сохранены формулы, обозначения, единицы, условия применения, типичные ошибки и вопросы для повторения.
- Карточка связана с самостоятельными Physics concepts и не заменяет их.

## 2026-07-22 (KNOWLEDGE_EXPORT sync: Physics — Математический аппарат физики)

### ✅ Синхронизировано

- Экспорт импортирован в подтверждённую Knowledge Area [[Physics/Physics|Physics]].
- Созданы: [[Physics/Физическая величина|Физическая величина]], [[Physics/Размерность физической величины|Размерность физической величины]], [[Physics/Единицы измерения и система SI|Единицы измерения и система SI]], [[Physics/Скалярные и векторные величины|Скалярные и векторные величины]], [[Physics/Координатная система и компоненты вектора|Координатная система и компоненты вектора]], [[Physics/Изменение величины и символ дельта|Изменение величины и символ дельта]], [[Physics/Производная как скорость изменения|Производная как скорость изменения]], [[Physics/Интеграл как накопление|Интеграл как накопление]], [[Physics/Дифференциальное уравнение|Дифференциальное уравнение]] и [[Physics/Численное накопление состояния|Численное накопление состояния]].
- Сводная тема «Формулы — математический аппарат физики» первоначально была пропущена как дублирующий контейнер, а затем создана отдельной учебной карточкой по явному запросу пользователя (см. запись выше).
- Общая [[Machine Learning/Mathematics/Производная|Производная]] не дублировалась; Physics note хранит доменную интерпретацию производной как скорости изменения.

### 🧠 Weak spots

- Интуитивный и формальный смысл производной; различие `Δ` и `d`.
- Интеграл как непрерывное накопление и его связь с производной.
- Дифференциальное уравнение как задача поиска функции.
- Переход от математической записи к численной реализации.

### Повторяющиеся ошибки

- Смешение скорости и ускорения.
- Попытка вычислить ускорение как `s/t²` без достаточных условий.
- Неуверенность в физическом типе результата вычисления.
- Интерпретация знака вектора без выбора координатной оси.

### Следующий шаг

- Разобрать графики `x(t)`, `v(t)` и `a(t)`, а затем реализовать дискретное накопление `velocity` и `position` методом Эйлера в Python.
