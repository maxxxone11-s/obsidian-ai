...
При импорте **KNOWLEDGE_EXPORT** агент обязан обновлять Roadmap не только по отдельным концепциям, но и по модулю целиком: отмечать статус модуля, общий прогресс, завершение/активность следующего этапа и темы, требующие повторения.
