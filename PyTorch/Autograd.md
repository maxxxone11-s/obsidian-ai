...
Autograd строит вычислительный граф во время forward и автоматически считает производные при `backward()`.
...
`backward()` проходит по графу в обратном направлении, использует сохраненные промежуточные значения и вычисляет градиенты обучаемых параметров. Он не запускает новый forward.
...
- Думать, что `backward()` снова запускает модель.
- Путать computation graph с уже накопленными градиентами.
...
- Когда строится вычислительный граф?
- Что хранится в computation graph?
